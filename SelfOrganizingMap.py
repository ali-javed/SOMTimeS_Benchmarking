...
class SelfOrganizingMap:
    ...
    def bmu(self, x, windowSize, candidateWeights):
        ...
        for i in candidateWeights:
            ...
            # get dtw distance
            try:
                distance = dtw(x = xCopy, y = weights, dist = 'square')
            except:
                distance = float('inf')
            #distance = 
    ...
    def propogateForward(self, inputs, windowSize, bounding):
        ############
        # Description: Wraper funding to allow and disallow dtw bounding
        # x: single input
        ...
        # if user wants to use SOMTimeS's bounding approach to speed up the process. There is no reason not to use bounding. This option is available for testing and validating speed-up.

        if bounding:
            inputs_arr = inputs[0]
            env_ups = inputs[1]
            env_lows = inputs[2]
            return self.propogateForward_pruned_approach(inputs_arr, windowSize,env_ups,env_lows)
        else:
            return self.propogateForward_dtw(inputs, windowSize)

    def propogateForward_dtw(self, inputs, windowSize):
        ############
        # Description: Function forward propogates from input to grid that DOES NOT use pruning
        # x: single input
        ...
        for i in range(0, len(inputs)):
            ...
            dtw_distances = []
            xCopy = copy.deepcopy(inputs[i])
            for j in range(0, len(self.weights_Kohonen)):
                # dtw_distances.append(random.random())
                # DTW diat seems to have a "issue" where it tries write access on inputs
                weights = copy.deepcopy(self.weights_Kohonen[j])

                #distance = dtaidistance.dtw.distance_fast(xCopy, weights, window=windowSize, use_pruning=False)
                #try:    
            ...
            bmus.append(np.argmin(dtw_distances))

        return bmus, dtw_Cals

    def propogateForward_pruned_approach(self, inputs, windowSize,env_ups,env_lows):
        ############
        ...
        for i in range(0, len(inputs)):
            ...
            lower_bounds = []
            for j in range(0, len(self.weights_Kohonen)):
                lower_bounds.append(metrics.lb_keogh(ts_query = self.weights_Kohonen[j], radius=windowSize,envelope_candidate=(env_lows[i], env_ups[i])))#inputs[i],
            ...
            if upper_bounds[i] <= min(lower_bounds):
                ...
                ind = np.argmin(dtw_distances)
                bmus.append(candidateWeights[ind])
                    # update min distance if new distance is lower
                    #if distance < min_distance:
                    #    bmuIndex = j
                    #    min_distance = distance
    ...
    def iterate(self, inputs, epochs, windowSize=0, k=1, randomInitilization=True, bounding=True):
        ############
        ...
        # windowSize: windowSize to be used by DTW (for project)
        ...
        if randomInitilization:
            for i in range(0, len(inputs[0])):
                ...
                secondPart = (np.mean(np.asarray(inputs)[:, i])) * np.random.uniform(low=min_test, high=max_test,
                                                                                     size=(len(self.weights_Kohonen)))
        ...
        if bounding:
            for i in inputs:
                ...
                env_low, env_up = lb_envelope(i, radius=windowSize)
                env_lows.append(env_low)
                env_ups.append(env_up)

        env_lows = np.asarray(env_lows)
        env_ups = np.asarray(env_ups)
        ...
        for epoch in range(0, epochs):
            ...
            if bounding:
                env_lows_p = np.array_split(env_lows, num_cores)
                env_ups_p = np.array_split(env_ups, num_cores)
                inputs_and_env = []
                for i in range(0,len(inputs_p)):
                    temp = [inputs_p[i],env_ups_p[i],env_lows_p[i]]
                    inputs_and_env.append(temp)

            #here merge
            if bounding:
                returnValues = Parallel(n_jobs=num_cores)(delayed(self.propogateForward)(i, windowSize, bounding) for i in inputs_and_env)
            else:
                returnValues = Parallel(n_jobs=num_cores)(delayed(self.propogateForward)(i, windowSize, bounding) for i in inputs_p)
            ...
            #bmuIndices,calls = self.propogateForward(inputs,windowSize,bounding=True)
        ...
        if k > 1:
            ...
            for a in range(0, len(self.weights_Kohonen)):
                for b in range(a + 1, len(self.weights_Kohonen)):
                    distance_matrix[a, b] = dtaidistance.dtw.distance_fast(self.weights_Kohonen[a], self.weights_Kohonen[b],
                                                              window=windowSize, use_pruning=True)
            ...
            clustering = SpectralClustering(n_clusters=k, n_init=100, assign_labels='discretize', random_state=0,
    ...
    def createUmatrix(self, windowSize):
        ...
        for idx in range(0, len(self.weights_Kohonen)):
            ...
            for neighbor in neighbors:
                ...
                distance = dtaidistance.dtw.distance_fast(neighborWeights, nodeWeights, window=windowSize, use_pruning=True)
    ...
    def locationOnMap(self, inputs, windowSize):
        locations = []
        inputs = np.asarray(inputs)
        bmuIndices, dtwCalls = self.propogateForward(inputs, windowSize, bounding=True)
    ...
    def plotMap(self, inputs, windowSize, labels=[], labelNames={}, path='plot_epoch'):
        ...
        for label in uniqueLabels:
            ...
            bmuIndices, dtwCalls = self.propogateForward(inputs_p, windowSize, bounding=True)
